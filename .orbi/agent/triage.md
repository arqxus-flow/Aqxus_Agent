...
**Only** add if it's specifically about the Desktop application or `orbi web` view. **Do not** add for terminal, TUI, or general orbi issues.
...
**Only** add if the issue mentions "zen" or "orbi zen" or "orbi black".

If the issue doesn't have "zen" or "orbi black" in it then don't add zen label

#### core

Use for core server issues in `packages/orbi/`, excluding `packages/orbi/src/cli/cmd/tui/`.
...
TUI (`packages/orbi/src/cli/cmd/tui/...`):
...
Core (`packages/orbi/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for orbi server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
