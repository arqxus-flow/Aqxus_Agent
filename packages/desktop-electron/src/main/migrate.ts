...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "com.orbi.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `orbi.settings.dat` is special: it maps to the `orbi.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"orbi.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // orbi.settings.dat → the electron settings store ("orbi.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "orbi.global.dat").
  const storeName = filename === "orbi.settings.dat" ? "orbi.settings" : filename
