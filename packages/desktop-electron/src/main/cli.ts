...
const CLI_INSTALL_DIR = ".orbi/bin"
const CLI_BINARY_NAME = "orbi"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `orbi-cli${suffix}`)
    : join(root, "../../resources", `orbi-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "orbi-install.sh")
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    ORBI_SERVER_USERNAME: "orbi",
    ORBI_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    ORBI_EXPERIMENTAL_ICON_DISCOVERY: "true",
    ORBI_EXPERIMENTAL_FILEWATCHER: "true",
    ORBI_CLIENT: "desktop",
...
function buildCommand(args: string, env: Record<string, string>) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.orbi/bin/orbi"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://orbicowork.arqxus.com/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",
