...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("orbi-find")
  api.delete("orbi-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("orbi-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("orbi-find", new Highlight(...rest))
    return true
  }
