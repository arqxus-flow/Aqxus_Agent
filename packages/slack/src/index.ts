...
import { createOpencode, type ToolPart } from "@orbi/sdk"
...
console.log("🚀 Starting orbi server...")
const orbi = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await orbi.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new orbi session...")
    const { client, server } = orbi
    ...
    console.log("✅ Created orbi session:", createResult.data.id)
  ...
  console.log("📝 Sending to orbi:", message.text)
