...
const CLI_INSTALL_DIR: &str = ".orbi/bin";
const CLI_BINARY_NAME: &str = "orbi";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("orbi-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "ORBI_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "ORBI_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("ORBI_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.orbi/bin/orbi\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://orbicowork.arqxus.com/install | bash -s -- --version {} --no-modify-path",
            ...
            let mut env_prefix = vec![
                "ORBI_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "ORBI_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "ORBI_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "ORBI_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "ORBI_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "ORBI_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("ORBI_SERVER_USERNAME", "orbi".to_string()),
        ("ORBI_SERVER_PASSWORD", password.to_string()),
    ];
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("ORBI_CLIENT"), Some(&"desktop".to_string()));
    }
