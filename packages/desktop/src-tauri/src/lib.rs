...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_done = needs_migration.then(|| {
        tracing::info!(
            path = %orbi_db_path().expect("failed to get db path").display(),
...
fn get_sidecar_port() -> u32 {
    option_env!("ORBI_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("ORBI_PORT").ok())
...
fn orbi_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("orbi").join("orbi.db"))
}
