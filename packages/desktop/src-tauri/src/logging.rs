...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("orbi-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("orbi_lib=debug,orbi_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("orbi_lib=info,orbi_desktop=info,sidecar=info")
        }
    });
