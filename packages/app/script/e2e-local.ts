...
const orbiDir = path.join(repoDir, "packages", "orbi")
...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "orbi-e2e-"))
const keepSandbox = process.env.ORBI_E2E_KEEP_SANDBOX === "1"

const serverEnv = {
  ...process.env,
  ORBI_DISABLE_SHARE: process.env.ORBI_DISABLE_SHARE ?? "true",
  ORBI_DISABLE_LSP_DOWNLOAD: "true",
  ORBI_DISABLE_DEFAULT_PLUGINS: "true",
  ORBI_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
  ORBI_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  ORBI_E2E_PROJECT_DIR: repoDir,
  ORBI_E2E_SESSION_TITLE: "E2E Session",
  ORBI_E2E_MESSAGE: "Seeded for UI e2e",
  ORBI_E2E_MODEL: "orbi/gpt-5-nano",
  ORBI_CLIENT: "app",
  ORBI_STRICT_CONFIG_DEPS: "true",
...
const runnerEnv = {
  ...
  VITE_ORBI_SERVER_HOST: "127.0.0.1",
  VITE_ORBI_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    const log = await import("../../orbi/src/util/log")
    const install = await import("../../orbi/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../orbi/src/server/server")
    inst = await import("../../orbi/src/project/instance")
    server = servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`orbi server listening on http://127.0.0.1:${serverPort}`)
