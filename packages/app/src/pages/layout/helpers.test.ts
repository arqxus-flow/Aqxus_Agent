...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("orbi://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("orbi://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("orbi://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("orbi://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("orbi://open-project")).toBeUndefined()
    expect(parseDeepLink("orbi://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "orbi://open-project?directory=/a",
      "orbi://other?directory=/b",
      "orbi://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("orbi://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("orbi://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("orbi://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("orbi://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "orbi://new-session?directory=/a",
      "orbi://open-project?directory=/b",
      "orbi://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __ORBI__: {
        deepLinks: ["orbi://open-project?directory=/a"],
      },
    } as unknown as Window & { __ORBI__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["orbi://open-project?directory=/a"])
