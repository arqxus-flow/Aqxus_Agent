...
const TUI_SCHEMA_URL = "https://orbicowork.arqxus.com/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from orbi.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const orbi = await orbiFiles(input)
  for (const file of orbi) {
...
async function orbiFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.ORBI_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("orbi", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "orbi")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "orbi"))
  }
  if (Flag.ORBI_CONFIG) files.push(Flag.ORBI_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "orbi"))
