...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for orbi.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal orbi sessions
...
```bash
ORBI_ENABLE_QUESTION_TOOL=1 orbi acp
```
...
- **Terminal Integration**: Full terminal support via orbi's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | orbi acp
```
...
ACP sessions map cleanly to orbi's internal session model:
