...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const orbiModel = {
      ...
      api: {
        id: "orbi-test",
        url: "https://api.orbicowork.arqxus.com",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, orbiModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.orbi?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.orbi?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.orbi?.itemId).toBe("msg_orbi")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.orbi?.itemId).toBe("msg_orbi_part")
