...
const managedConfigDir = process.env.ORBI_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "orbi.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "orbi.json") {
...
test("ignores legacy tui keys in orbi config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://orbicowork.arqxus.com/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "orbi.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["ORBI_CONSOLE_TOKEN"]
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["orbi"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["ORBI_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["ORBI_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "orbi.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .orbi directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const orbiDir = path.join(dir, ".orbi")
      await fs.mkdir(orbiDir, { recursive: true })
      const agentDir = path.join(orbiDir, "agent")
...
test("loads agents from .orbi/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const orbiDir = path.join(dir, ".orbi")
      await fs.mkdir(orbiDir, { recursive: true })

      const agentsDir = path.join(orbiDir, "agents")
...
test("loads commands from .orbi/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const orbiDir = path.join(dir, ".orbi")
      await fs.mkdir(orbiDir, { recursive: true })

      const commandDir = path.join(orbiDir, "command")
...
test("loads commands from .orbi/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const orbiDir = path.join(dir, ".orbi")
      await fs.mkdir(orbiDir, { recursive: true })

      const commandsDir = path.join(orbiDir, "commands")
...
test("does not try to install dependencies in read-only ORBI_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.ORBI_CONFIG_DIR
  process.env.ORBI_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ORBI_CONFIG_DIR
    else process.env.ORBI_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable ORBI_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "orbi.json"),
        JSON.stringify({ $schema: "https://orbicowork.arqxus.com/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "orbi.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .orbi config
      const projectDir = path.join(dir, "project")
      const orbiDir = path.join(projectDir, ".orbi")
      await fs.mkdir(orbiDir, { recursive: true })
      ...
      // Local .orbi config with different plugins
      await Filesystem.write(
        path.join(orbiDir, "orbi.json"),
        JSON.stringify({
          $schema: "https://orbicowork.arqxus.com/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .orbi config with some overlapping plugins
      await Filesystem.write(
        path.join(orbiDir, "orbi.json"),
        JSON.stringify({
          $schema: "https://orbicowork.arqxus.com/config.json",
...
// Note: preload.ts sets ORBI_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .orbi config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .orbi directory config enables it
      const orbiDir = path.join(dir, ".orbi")
      await fs.mkdir(orbiDir, { recursive: true })
      await Filesystem.write(
        path.join(orbiDir, "orbi.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/orbi")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/orbi")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-orbi@2.4.3")).toBe("oh-my-orbi")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@orbi/plugin@2.0.0")).toBe("@orbi/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-orbi@2.4.3", "file:///project/.orbi/plugin/oh-my-orbi.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.orbi/plugin/oh-my-orbi.js")
  })
  ...
  test("local plugin directory overrides global orbi.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const orbiDir = path.join(projectDir, ".orbi")
        const pluginDir = path.join(orbiDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("ORBI_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["ORBI_DISABLE_PROJECT_CONFIG"]
    process.env["ORBI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["ORBI_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["ORBI_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .orbi/ directories when flag is set", async () => {
    const originalEnv = process.env["ORBI_DISABLE_PROJECT_CONFIG"]
    process.env["ORBI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .orbi directory with a command
          const orbiDir = path.join(dir, ".orbi", "command")
          await fs.mkdir(orbiDir, { recursive: true })
          await Filesystem.write(path.join(orbiDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .orbi should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["ORBI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ORBI_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["ORBI_CONFIG_DIR"]
      process.env["ORBI_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["ORBI_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["ORBI_CONFIG_DIR"]
      } else {
        process.env["ORBI_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("ORBI_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["ORBI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ORBI_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["ORBI_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["ORBI_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from ORBI_CONFIG_DIR, not project
...
describe("ORBI_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in ORBI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ORBI_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["ORBI_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://orbicowork.arqxus.com/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["ORBI_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["ORBI_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in ORBI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ORBI_CONFIG_CONTENT"]
