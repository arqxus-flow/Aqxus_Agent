...
test("discovers skills from .orbi/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".orbi", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".orbi", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".orbi", "skill", "dir-skill")
...
test("discovers multiple skills from .orbi/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".orbi", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".orbi", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".orbi", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.ORBI_TEST_HOME
  process.env.ORBI_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.ORBI_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const orbiSkillDir = path.join(dir, ".orbi", "skill", "agent-skill")
      const orbiSkillsDir = path.join(dir, ".orbi", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(orbiSkillDir, "SKILL.md"),
        `---
name: orbi-skill
description: A skill in the .orbi/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(orbiSkillsDir, "SKILL.md"),
        `---
name: orbi-skill
description: A skill in the .orbi/skills directory.
---
