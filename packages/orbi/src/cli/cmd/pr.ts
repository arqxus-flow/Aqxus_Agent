...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run orbi",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for orbi session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found orbi session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["orbi", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting orbi...")
        UI.println()

        const orbiArgs = sessionId ? ["-s", sessionId] : []
        const orbiProcess = Process.spawn(["orbi", ...orbiArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await orbiProcess.exited
        if (code !== 0) throw new Error(`orbi exited with code ${code}`)
      },
    })
  },
})
