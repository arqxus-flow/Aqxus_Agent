...
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@orbi/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run orbi with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running orbi server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to ORBI_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.ORBI_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.ORBI_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.ORBI_SERVER_USERNAME ?? "orbi"
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://orbi.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
