...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.ORBI_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.ORBI_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.ORBI_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".orbi") && dir !== Flag.ORBI_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old orbi.json shape) still get their settings applied.
    const normalized = (() => {
