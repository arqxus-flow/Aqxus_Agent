import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@orbi/sdk/v2"
import { SessionTurn } from "@orbi/ui/session-turn"
import { SessionReview } from "@orbi/ui/session-review"
import { DataProvider } from "@orbi/ui/context"
import { FileComponentProvider } from "@orbi/ui/context/file"
import { WorkerPoolProvider } from "@orbi/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@orbi/ui/logo"
import { IconButton } from "@orbi/ui/icon-button"
import { ProviderIcon } from "@orbi/ui/provider-icon"
import { iife } from "@orbi/util/iife"
import { Binary } from "@orbi/util/binary"
...
import { FileSSR } from "@orbi/ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@orbi/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/orbi-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Orbi</Title>
              </Show>
              <Meta name="description" content="orbi - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/anomalyco/orbi"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://orbicowork.arqxus.com/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
