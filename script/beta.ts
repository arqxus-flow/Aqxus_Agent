...
async function fix(pr: PR, files: string[]) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with orbi...`)
  const prompt = [
  ...
  try {
    await $`orbi run -m orbi/gpt-5.3-codex ${prompt}`
  } catch (err) {
    console.log(`  orbi failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with orbi")
  return true
}
